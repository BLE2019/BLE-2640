...
void i2c_init(void)
{
	...
	cc2640_I2C_Params.bitRate = I2C_400kHz; //高速i2c模式
...
static void i2c_write(size_t slaveAddress,uint8_t *write_data,size_t len)
{
	 ...
	 cc26xxI2C_config[CC2640R2_LAUNCHXL_I2C0].fxnTablePtr->transferFxn(cc2640_I2C_Handle,&transaction);
}
...
static void i2c_read(uint8_t slaveAddress,uint8_t *read_data,uint8_t len)
{
	 ...
	 transaction.arg = NULL;
	if (cc26xxI2C_config[CC2640R2_LAUNCHXL_I2C0].fxnTablePtr->transferFxn(cc2640_I2C_Handle,&transaction))
	{
            //HwUARTWrite("i2c read success\r\n",18);
	}
}
