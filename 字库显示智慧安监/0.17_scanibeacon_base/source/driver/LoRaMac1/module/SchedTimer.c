...
/* The timer list pointed to by "TimerListHead" was accessed by external functions:
     (1) TimerStart()
     (2) TimerStop()
     (3) TimerIrqHandler()
     the (1) and (2) called by ISR and PROCESS
     so need DISABLE/ENABLE IRQ to avoid race condition! */
    #if 0 //0706!!!!!!!!!!!!!
//static halIntState_t    s_intState; //0706
#define BoardDisableIrq()    HAL_ENTER_CRITICAL_SECTION(s_intState)
#define BoardEnableIrq()    HAL_EXIT_CRITICAL_SECTION(s_intState)
#else
...
#if EXTI_HALT_UART
    /* Insure "Agree Halt by RTC" after this ISR have completed. */
...
#if EXTI_HALT_UART
    pm_ExitLowPwr(); /* Set MCU to RUN_MODE for Radio */
#endif
...
{
 //   return RtcGetTimerValue( );  //0706
 	return (TimerTime_t)0;
}

TimerTime_t TimerGetElapsedTime( TimerTime_t savedTime )
{
  //  return RtcComputeElapsedTime( savedTime ); //0706;
      return (TimerTime_t)0;
...
{
 //   return RtcComputeFutureEventTime( eventInFuture ); //0706
  	return (TimerTime_t)0;
