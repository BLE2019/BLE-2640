...
//#include "Dbg.h"
/* Compile switch-------------------------------------------------------------*/
#define TX_DBG    RIME_DBG_ON
#define RX_DBG    RIME_DBG_ON
#define TX_MAC_CMD_DBG    RIME_DBG_ON
#define RX_MAC_CMD_DBG    RIME_DBG_ON

#if REL_VER
#undef TX_DBG
#undef RX_DBG
#undef TX_MAC_CMD_DBG
#undef RX_MAC_CMD_DBG
#define TX_DBG    RIME_DBG_OFF
#define RX_DBG    RIME_DBG_OFF
#define TX_MAC_CMD_DBG    RIME_DBG_OFF
#define RX_MAC_CMD_DBG    RIME_DBG_OFF
#endif

#if (RIME_DBG_ON == RX_DBG)
static uint32_t    s_ulRxFreq;
#endif
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
 ...
 * Tx output powers table definition
 */
const int8_t TxPowers[]    = { 17, 16, 14, 12, 10, 7, 5, 2, 20 };
...
/*!
 * Defines the number of the channels enabled
 */
#define EN_CH_NUM       MIN(16, LORA_MAX_NB_CHANNELS)

/*!
 * Defines the offset of the channels enabled
 */
#define EN_CH_OFFSET        (LORA_CN470_START_CH % 16)
...
#if 0  //0709!!!!!!!!!!!!!!!
#if (RIME_DBG_ON == RX_DBG)
    RIME_DBG2(RX_DBG, "\r\n\r\nrxpk: freq=", s_ulRxFreq, PRINTF_FORMAT_DEC);
    RIME_DBG2(RX_DBG, ", datr=SF", Datarates[McpsIndication.RxDatarate], PRINTF_FORMAT_DEC);
    RIME_DBG2(RX_DBG, "BW125, codr=4/5, rssi=", rssi, PRINTF_FORMAT_DEC);
    RIME_DBG2(RX_DBG, ", lsnr=", snr, PRINTF_FORMAT_DEC);
    RIME_DBG2(RX_DBG, ", size=", size, PRINTF_FORMAT_DEC);
    RIME_DBG2(RX_DBG, "\r\n", 0, PRINTF_FORMAT_NONE);
    dbg_PrintfBufData(payload, size);
#endif
#endif
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
...
#elif defined( USE_BAND_470 )
    RxWindowSetup( LORAMAC_FIRST_RX1_CHANNEL + ( (Channel + LORA_CN470_START_CH) % 48 ) * LORAMAC_STEPWIDTH_RX1_CHANNEL, RxWindowsParams[0].Datarate, RxWindowsParams[0].Bandwidth, RxWindowsParams[0].RxWindowTimeout, false );
#elif ( defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID ) )
...
static bool SetNextChannel( TimerTime_t* time )
{
	...
	for( uint8_t i = 0; i < LORA_MAX_NB_BANDS; i++ )
	{
        ...
        for( uint8_t i = 0, k = LORA_CN470_START_CH / 16; i < LORA_MAX_NB_CHANNELS; i += 16, k++ )
        {
            for( uint8_t j = 0; j < EN_CH_NUM; j++ )
#else
...
#elif defined( USE_BAND_470 )
                if( ( LoRaMacParams.ChannelsMask[k] & ( 1 << (j + EN_CH_OFFSET) ) ) != 0 )
#else
...
static bool ValidateDatarate( int8_t datarate, uint16_t* channelsMask )
{
    ...
    for( uint8_t i = 0, k = 0; i < LORA_MAX_NB_CHANNELS; i += 16, k++ )
    {
        ...
        {
#if defined( USE_BAND_470 )
            if( ( channelsMask[k] & ( 1 << (j + EN_CH_OFFSET) ) ) != 0 )
#else
...
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
#if 0  //0709
#if (RIME_DBG_ON == RX_MAC_CMD_DBG)
    RIME_DBG2(TX_DBG, "\r\n\r\nrx mac cmd, size=", (commandsSize - macIndex), PRINTF_FORMAT_DEC);
    RIME_DBG2(TX_DBG, "\r\n", 0, PRINTF_FORMAT_NONE);
    dbg_PrintfBufData(&payload[macIndex], (commandsSize - macIndex));
#endif
#endif
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
	#endif //0725!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            case SRV_MAC_LINK_ADR_REQ:
                {
                    ...
                    if( chMaskCntl == 6 )
                    {
                        ...
                        for( uint8_t i = 0, k = LORA_CN470_START_CH / 16; i < LORA_MAX_NB_CHANNELS; i += 16, k++ )
                        {
                            for( uint8_t j = 0; j < EN_CH_NUM; j++ )
                            {
                                if( Channels[i + j].Frequency != 0 )
                                {
                                    channelsMask[k] |= 1 << (j + EN_CH_OFFSET);
                                }
                            }
                        }
                    }
                    ...
                    else if( chMaskCntl != ( LORA_CN470_START_CH / 16 ) )
                    {
                        if( chMask != 0 )// Trying to enable an undefined channel
                        {
                            status &= 0xFE; // Channel mask KO
                        }
                        else
                    ...
                    else//chMaskCntl == ( LORA_CN470_START_CH / 16 )
                    {
                        for( uint8_t i = 0; i < EN_CH_NUM; i++ )
                        {
                            if( ( ( chMask & ( 1 << (i + EN_CH_OFFSET) ) ) != 0 ) &&
                                ( Channels[i].Frequency == 0 ) )
                            {// Trying to enable an undefined channel
                                status &= 0xFE; // Channel mask KO
                            }
...
#if defined( USE_BAND_470 )
    if(TX_POWER_20_DBM > LoRaMacParams.ChannelsTxPower)
    {
        LoRaMacParams.ChannelsTxPower = MIN(LoRaMacParams.ChannelsTxPower, LoRaMacParamsDefaults.ChannelsTxPower);
    }
#else
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        /*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
#if 0
        #if (RIME_DBG_ON == TX_MAC_CMD_DBG)
            if (0 < MacCommandsBufferIndex)
            {
                RIME_DBG2(TX_DBG, "\r\n\r\ntx mac cmd, size=", MacCommandsBufferIndex, PRINTF_FORMAT_DEC);
                RIME_DBG2(TX_DBG, "\r\n", 0, PRINTF_FORMAT_NONE);
                dbg_PrintfBufData(MacCommandsBuffer, MacCommandsBufferIndex);                
            }
        #endif
#endif
        /*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
    Radio.SetTxConfig( MODEM_LORA, txPower, 0, 0, datarate, 1, 8, false, true, 0, 0, false, 3e3 );
    TxTimeOnAir = Radio.TimeOnAir( MODEM_LORA, LoRaMacBufferPktLen );  //´Ë´¦Î´µ÷ÓÃRTC£¬²»¸Ä
#else
    #error "Please define a frequency band in the compiler options."
#endif
...
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
#if 0  //0709
#if (TX_DBG == RIME_DBG_ON)
    RIME_DBG2(TX_DBG, "\r\n\r\ntxpk: freq=", channel.Frequency, PRINTF_FORMAT_DEC);
    RIME_DBG2(TX_DBG, ", powe=", txPower, PRINTF_FORMAT_DEC);
    RIME_DBG2(TX_DBG, ", datr=SF", datarate, PRINTF_FORMAT_DEC);
    RIME_DBG2(TX_DBG, "BW125, codr=4/5, size=", LoRaMacBufferPktLen, PRINTF_FORMAT_DEC);
    RIME_DBG2(TX_DBG, "\r\n", 0, PRINTF_FORMAT_NONE);
    dbg_PrintfBufData(LoRaMacBuffer, LoRaMacBufferPktLen);
#endif
#endif
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
    ...
    LoRaMacState |= LORAMAC_TX_RUNNING; //¿ÉÒÔÔÝÊ±¸ÄÎ´IDLE£¬¹æ±Ü×´Ì¬ÅÐ¶Ï£¬Ö±½Ó·¢
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    {
        Channels[i].Frequency = 470.3e6 + (i + LORA_CN470_START_CH) * 200e3;
