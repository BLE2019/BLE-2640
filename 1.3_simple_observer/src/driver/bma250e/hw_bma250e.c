...
uint16_t bufXYZ[3] = {0, 0, 0};
uint16_t Count_step = 0;
...
void BMA250E_IDTest(void)
{
    bool status = false;
    uint8_t rxbuf[10];
    status = HwI2CGet(0x18,0x00, rxbuf, 1);
    if(status == true) {
        if(rxbuf[0] == 0xF9) {
...
{
    //BMA250E_IDTest();
    HwI2CSet(0x18,0x0F, 0x03); //加速度范围+-2g
    HwI2CSet(0x18,0x10, 0x08); //带宽7.8Hz
    HwI2CSet(0x18,0x09, 0x00);  //清中断位
    HwI2CSet(0x18,0x16, 0xff);  //允许中断
    HwI2CSet(0x18,0x17, 0xff);  //允许中断
    HwI2CSet(0x18,0x19, 0xff);  //允许中断
...
bool BMA250E_GetData(void)
{
    ...
    ret = HwI2CGet(0x18,0x02, buffer, 6);
...
{
    bool  ret = false;
    ret = HwI2CGet(0x18,0x11, &SleepMode, 1);
    return ret;
}
...
bool BMA250E_SetSleep(bool sleep)
{
    ...
    ret = HwI2CSet(0x18,0x11, SleepMode);
...
void BMA250E_SetRange(uint8_t range)
{
    ...
    HwI2CSet(0x18,0x0F, flag);
}

void CountStep(void)
{
    static uint8_t i;
    static uint16_t CountXYZ[240];
    for(i = 0;i < 80 ;i++)
    {
        BMA250E_GetData();
        CountXYZ[i*3+0] = bufXYZ[0];
        CountXYZ[i*3+1] = bufXYZ[1];
        CountXYZ[i*3+2] = bufXYZ[2];
    }
    Count_step += count_steps(CountXYZ);
}
